...
**Status meaning:** a task is `done` only when its PR is **merged to master**; a task with an open PR is `in-progress`.
...
| T4 | `ChatMessage.images` + ctor extraction + `format_tool_result` arm + `elide_binary_payload` | Mid | done | T3 | 2 MB `view_image` row ⇒ `tool_result.len() < 512`, `images` intact, `content == "🖼 shot.png"`; bash row untouched; elide is a fixpoint | #313 (merged) |
| T5 | `images` on `conversation::Message::ToolResult` + both sync arms | Junior | pending | T4 | Ref survives `ChatMessage → ConvMsg → JSON → back`; old JSON without the key still parses; no-image rows serialize byte-identically — **MUST land before T6/T7** | |
