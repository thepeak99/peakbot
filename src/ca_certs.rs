...
//!
//! reqwest 0.13 uses `rustls-platform-verifier`, whose Unix backend reads
//! roots from the system store (honouring `SSL_CERT_FILE`/`SSL_CERT_DIR`).
//! A static musl binary on a bare Android/Termux device has neither, so
//! `Client::new()` panics ("No CA certificates were loaded from the system").
//! We write the compiled-in Mozilla bundle to the cache dir once and point
//! `SSL_CERT_FILE` at it — before any TLS client is built — so every reqwest
//! client (ours, rig, rmcp, spider) finds roots transitively.
...
///
/// No-op when the operator already set `SSL_CERT_FILE` or `SSL_CERT_DIR`, or
/// when a standard system bundle exists — we never override real roots.
...
/// Common distro locations for a system CA bundle. If one exists, the device
/// already has roots and platform-verifier will find them on its own.
